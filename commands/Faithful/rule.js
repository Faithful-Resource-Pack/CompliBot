...
module.exports = {
	...
	async execute(client, message, args) {
		if (
			!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR) || // managers
			!message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES) // moderators
		)
		...
		const thumbnail =
			message.guild.id == settings.guilds.classic_faithful
		...
		// number is out of range
			...
			return warnUser(
				message,
				`You have to specify a number between 1 and ${RULES.length} included.`,
			);
	},
};
