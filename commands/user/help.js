...
module.exports = {
	...
	async execute(client, message, args) {
		...
		if (!args[0]) {
			...
			if (message.guild !== null && message.member.roles.cache.some(role => role.name.includes("Administrator") || role.name.includes("Moderator") || role.id === '747839021421428776')) embed.addField("Moderators", commands_mods[0] === undefined ? "None" : commands_mods.join('\n') )
			if (message.guild !== null && message.member.roles.cache.some(role => role.name.includes("Administrator") || role.id === '747839021421428776')) embed.addField("Administrators", commands_admins[0] === undefined ? "None" : commands_admins.join('\n') )
