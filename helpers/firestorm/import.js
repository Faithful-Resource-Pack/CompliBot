...
const save = function () {
	return new Promise((resolve, reject) => {
		all.users
			.write_raw({})
			.then(() => all.users.setBulk(Object.keys(all_users), Object.values(all_users)))
			.then(console.log)
			.catch((err) => {
...
const build_from_files = async function () {
	...
	if (DEBUG) console.log("Started build of database");
	...
	old_moderation.forEach((m) => {
		...
		if (m.timeout) u.timeout = m.timeout;
		if (m.muted) u.muted = m.muted;
		if (m.warn) u.warns = m.warn;

		if (Object.keys(u).length) all_users[m.user] = u;
	});
	...
	javaContributions.forEach((contrib) => {
		...
		const resss = ["c32", "c64"];
		resss.forEach((res) => {
			if (!contrib[res] || !contrib[res].author) return;
	...
	wasBuilt = true;
	// if(DEBUG) console.log(module.exports)
	const res = await save();
	console.log(module.exports);
	if (DEBUG) console.log("Ended build of database");
};
