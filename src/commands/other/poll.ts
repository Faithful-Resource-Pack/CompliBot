...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('question').setDescription('Write the question here.').setRequired(true))
    .addNumberOption((option) => option.setName('answers').setDescription('How many answers does the poll have? (Max: 5)').setRequired(true))
    ...
    .addStringOption((option) => option.setName('description').setDescription('Add more information about your poll here.')),
  execute: async (interaction: CommandInteraction) => {
    const question: string = interaction.options.getString('question', true);
    const multipleAnswers: boolean = interaction.options.getBoolean('allow-multiple-answers', false) === true;
    const timeoutVal: string | null = interaction.options.getString('timeout', false);
    const yesno: boolean = interaction.options.getBoolean('yesno', false) === true;
    const thread: boolean = interaction.options.getBoolean('thread', false) === true;
    const description: string = interaction.options.getString('description', false);
    const anonymous: boolean = interaction.options.getBoolean('anonymous', false) === true;

    const count: number = interaction.options.getNumber('answers', true);
    const answersCount: number = Math.min(count, 5) >= 2 ? Math.min(count, 5) : 2;
