...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((audit) => audit
      .setName('audit')
      .setDescription('change the banlist')
      .addUserOption((userOpt) => userOpt.setName('subject').setDescription('The user to affect from using the bot').setRequired(true))
      .addBooleanOption((bool) => bool.setName('pardon').setDescription('Weather to undo an oopsie or not').setRequired(false))),
  execute: new Collection<string, SlashCommandI>()
    .set('audit', async (interaction: CommandInteraction, client: Client) => {
      ...
      if (
        ...
        interaction.followUp(await interaction.getEphemeralString({ string: 'Command.Botban.view.unbannable' }));
        return;
      }

      if (interaction.options.getBoolean('pardon')) banlist.ids.filter(async (v) => v !== victimID); // removes only the id of the victim
      else banlist.ids.push(victimID);
    ...
    .set('view', async (interaction: CommandInteraction) => {
      if (
        await interaction.perms({
          type: 'dev',
        })
      ...
      switch (interaction.options.getString('format')) {
        ...
        case 'emb':
          interaction.followUp({
            embeds: [new MessageEmbed().setTitle('Bot banned IDs:').setDescription(JSON.parse(buffer.toString('utf-8')).ids.join('\n'))],
            ephemeral: true,
          });
          break;
        case 'ment':
          interaction.followUp({
            embeds: [new MessageEmbed().setTitle('Bot banned Users:').setDescription(`<@${JSON.parse(buffer.toString('utf-8')).ids.join('>\n<@')}>`)],
            ephemeral: true,
          });
          break;
        case 'txt':
