...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => subcommand
      ...
      .addStringOption((option) => option.setName('command').setDescription('Returns usage for a specific command.').setRequired(false))),
    ...
    .set('bot', async (interaction: CommandInteraction, client: Client) => {
      ...
      const FieldTitles = (
        await interaction.getEphemeralString({
          string: 'Command.Stats.Embed.FieldTitles',
        })
      ).split('$,');
      ...
      const embed = new MessageEmbed()
        ...
        .addFields(
          {
            name: FieldTitles[0],
            value: client.tokens.prefix,
            inline: true,
          },
          {
            name: FieldTitles[1],
            value: duration(client.uptime).humanize(),
            inline: true,
          },
          {
            name: FieldTitles[2],
