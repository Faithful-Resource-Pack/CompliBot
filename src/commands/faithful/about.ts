...
export const command: SlashCommand = {
	...
	execute: async (interaction: CommandInteraction) => {
		...
		const response = (await axios.get(`https://api.faithfulpack.net/v2/users/${user.id}/contributions`)).data;
		...
		for (let i of response.slice(0, AMOUNT_TO_LOAD)) {
			...
			try {
				const response2 = (await axios.get(`https://api.faithfulpack.net/v2/textures/${i.texture}`)).data;
