	...
	execute: async (interaction: CommandInteraction, client: Client) => {
		...
		const embed = new MessageEmbed()
			...
			.addFields(
				...
				{ name: "Status", value: guildUser.presence?.status != undefined ? guildUser.presence?.status : "offline", inline: true },
				{ name: "Joined at", value: `<t:${moment(guildUser.joinedAt).utc().unix()}>\n<t:${moment(guildUser.joinedAt).utc().unix()}:R>`, inline: true },
				{ name: "Created at", value: `<t:${moment(user.createdTimestamp).utc().unix()}>\n<t:${moment(user.createdTimestamp).utc().unix()}:R>`, inline: true },
				{ name: "Boost status", value: guildUser.premiumSince ? `Since <t:${moment(guildUser.premiumSince).utc().unix()}>\n<t:${moment(guildUser.premiumSince).utc().unix()}:R>` : "Not boosting", inline: true },
