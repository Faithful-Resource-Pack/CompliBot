...
export async function quote(message: Message) {
    ...
    let matches = /https:\/\/(canary\.)?discord(app)?\.com\/channels\/([0-9]+)\/([0-9]+)\/([0-9]+)/g.exec(message.content);
	...
	if (quotedMsg.embeds[0] != undefined) {
		embed.setAuthor({ name: `Embed sent by ${quotedMsg.author.tag}`, iconURL: "https://database.compliancepack.net/images/bot/quote.png" });
		...
		if (quotedMsg.attachments.size > 0 && quotedMsg.attachments.first().url.match(/\.(jpeg|jpg|png|webp|gif)$/)) embed.setImage(quotedMsg.attachments.first().url);

		embed.setAuthor({ name: `Message sent by ${quotedMsg.author.tag}`, iconURL: "https://database.compliancepack.net/images/bot/quote.png" })
