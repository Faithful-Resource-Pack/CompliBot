...
export async function slashCommandImage(options: slashCommandImageOptions) {
  ...
  if (message === undefined) {
    const embed = new MessageEmbed()
      ...
      .setDescription((await options.interaction.text({ string: "Command.Images.NotFound" })).replace("%NUMBER%", options.limit.toString()));
  ...
  const embed = new MessageEmbed()
    .setTitle(options.response.title)
    .setImage(options.response.url)
  ...
  options.interaction.editReply({ files: [file], embeds: [embed] })
