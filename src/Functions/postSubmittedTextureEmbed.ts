...
export const postSubmitedTextureEmbed = async (message: Message, texture: any, file: MessageAttachment) => {
  ...
  const embed = new MessageEmbed()
    ...
    .setFooter({ text: message.author.id }) // used to authenticate the submitter (for message deletion)
  ...
  files.push(await magnifyAttachment({ 
    url: (await axios.get(`${config.apiUrl}textures/${texture.id}/url/default/${texture.paths[0].versions.sort(minecraftSorter).reverse()[0]}`)).request.res.responseUrl, 
    name: 'magnified.png' 
  }));

  message.channel.send({ embeds: [embed], files: [...files], components: [submissionButtonsClosed, submissionButtonsVotes]});
}