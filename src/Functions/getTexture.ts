...
export const getTexture = async (texture, resolution: number): Promise<MessageEmbed> => {
  ...
  } catch (err) { console.error(err) } // image url isn't valid (ex: 404)
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    tmp[edition].push(`\`[${versions.length > 1 ? `${versions[0]} — ${versions[versions.length - 1]}` : versions[0]}]\` ${paths[i].path}`);
  }

  Object.keys(tmp).forEach(edition => {
    if (tmp[edition] && tmp[edition] !== 0) embed.addField(edition.charAt(0).toLocaleUpperCase() + edition.slice(1), tmp[edition].map(el => el.replace('assets/minecraft/', '').replace('textures/', '')).join('\n'), false);
  })
