...
export async function paletteAttachment(options: options): Promise<[MessageAttachment, MessageEmbed]> {
	return getMeta(options.url)
		.then(async (dimension) => {
			...
			for (let x = 0; x < dimension.width; x++) for (let y = 0; y < dimension.height; y++) {
			...
			const attachment = new MessageAttachment(colorCanvas.toBuffer("image/png"), `${options.name ? options.name : "palette.png"}`);
