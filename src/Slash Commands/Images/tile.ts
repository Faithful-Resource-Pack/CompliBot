...
export const command: SlashCommand = {
  ...
  execute: async (interaction: CommandInteraction) => {
    ...
    let message = messages // get latest message with attachment
      ...
      .filter(m => m.attachments.size > 0 || (m.embeds[0] !== undefined && (m.embeds[0].thumbnail !== null || m.embeds[0].image !== null)))
      .first();

    if (message === undefined) {
      const embed = new MessageEmbed()
        .setTitle('No images found')
        .setDescription('No images were found in the 10 lasts messages, please submit one using a image url or attaching one to a new message, you can cancel this by sending anything else.')

      interaction.editReply({ embeds: [embed] });

      const filter = (m) => m.author.id === interaction.member.user.id;
      const collected = await interaction.channel.awaitMessages({ filter: filter, max: 1, time: 30000, errors: ["time"] })
