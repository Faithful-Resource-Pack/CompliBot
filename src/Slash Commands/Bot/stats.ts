...
export const command: SlashCommand = {
  ...
  execute: new Collection<string, SlashCommandI>()
    .set("bot", async (interaction: CommandInteraction, client: Client) => {
      ...
      client.guilds.cache.each((guild: Guild) => { sumMembers += guild.memberCount; })
    ...
    .set("command", async (interaction: CommandInteraction, client: Client) => {
      const embed = new MessageEmbed().setTitle(`\`/${interaction.options.getString("command", true)}\` has been used \`${getUsage(interaction.options.getString("command", true)) ?? '0'}\` times`);
      interaction.reply({ embeds: [embed] })
    })
}