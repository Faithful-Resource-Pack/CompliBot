import { deleteInteraction, pollDelete, pollVotes, pollYesNo } from "@helpers/buttons";
...
export class Poll extends TimedEmbed {
  ...
  public async updateEmbed(client: Client): Promise<void> {
    ...
    embed.fields = embed.fields.map((field: EmbedField, index: number) => {
      ...
      field.value = 
        votesCount[index-1] === 0 ? "No votes yet." :
        `> ${votesCount[index-1]} / ${votesCount.reduce((partialSum, a) => partialSum + a, 0)} (${((votesCount[index-1] / votesCount.reduce((partialSum, a) => partialSum + a, 0)) * 100).toFixed(2)}%)`;

      if (this.isAnonymous() === false) field.value += votes[index-1] && votes[index-1].length > 0 ? `\n<@!${votes[index-1].join('>, <@!')}>` : "";
  ...
  public async postSubmissionMessage(interaction: CommandInteraction, embed: MessageEmbed, options: PollOptions): Promise<void> {
    embed.setTitle(options.question)
    embed.setFooter({ text: `${this.id} | ${embed.footer.text}` });
    embed.addFields(options.answersArr.map((answer: string, index: number) => {
    ...
    const message: Message = await interaction.editReply({ embeds: [embed], components: [...components, new MessageActionRow().addComponents(pollDelete)] }) as any;
