 ...
 */
export async function generalChoiceEmbed(interaction: CommandInteraction, menuID: string, results: MessageSelectOptionData[]) {
 ...
 */
export async function textureChoiceEmbed(interaction: CommandInteraction, menuID: string, results: Textures, constValue: string) {
    const mappedResults: MessageSelectOptionData[] = [];
    for (let result of results) {
        mappedResults.push({
            label: `[#${result.id}] (${
                result.paths[0].versions.sort(minecraftSorter).reverse()[0]
            }) ${result.name}`,
