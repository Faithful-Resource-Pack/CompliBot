...
export async function animateAttachment(options: Options): Promise<MessageAttachment> {
  return getMeta(options.url)
    .then(async (dimensions) => {
      ...
      else for (let i = 0; i < dimensions.height / dimensions.width; i++)
      ...
      if (MCMETA.animation.interpolate) {
        ...
        for (let i = 0; i < frames.length; i++) {
          for (let y = 1; y <= limit; y++) {
            ...
            context.drawImage(
              baseCanvas,                            // image
              0, dimensions.width * frames[i].index, // sx, sy
              dimensions.width, dimensions.width,    // sWidth, sHeight
              0, 0,                                  // dx, dy
              canvas.width, canvas.width             // dWidth, dHeight
            )
            ...
            context.drawImage(
              baseCanvas,                                                  // image
              0, dimensions.width * frames[(i + 1) % frames.length].index, // sx, sy
              dimensions.width, dimensions.width,                          // sWidth, sHeight
              0, 0,                                                        // dx, dy
              canvas.width, canvas.width                                   // dWidth, dHeight
            )
