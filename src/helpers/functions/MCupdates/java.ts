...
export async function loadJavaVersions() {
	try {
		var { status, data: versions } = await axios.get('https://launchermeta.mojang.com/mc/game/version_manifest_v2.json');
	} catch (e) {
				...
				if (version.id.includes('pre')) {
					...
					if (res.status == 404) {
						for (pre; pre > 1; pre--) {
							...
							if (res.status !== 404) {
								await updateChannel.send({ content: `A new pre-release of Minecraft Java was just released: \`${version.id}\`\nhttps://www.minecraft.net/en-us/article/minecraft-${version.id.split('-')[0].replace(/\./g,'-')}-pre-release-${pre}` });
								break;
							}
						}
					}
					...
					await updateChannel.send({ content: `A new release candidate of Minecraft Java was just released: \`${version.id}\`\nhttps://www.minecraft.net/en-us/article/minecraft-${version.id.split('-')[0].replace(/\./g,'-')}-release-candidate-1` });
				}
				else {
					if (version.type === 'snapshot') await updateChannel.send({ content: `A new ${version.type} of Minecraft Java was just released: \`${version.id}\`\nhttps://www.minecraft.net/en-us/article/minecraft-snapshot-${version.id}` });
					else if (version.type === 'release') await updateChannel.send({ content: `A new ${version.type} of Minecraft Java was just released: \`${version.id}\`\nhttps://www.minecraft.net/en-us/article/minecraft-java-edition-${version.id.replace(/\./g,'-')}` });
				}
			}
		})
	} catch (e) {
